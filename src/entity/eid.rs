...
where
    IndexT: Succ + Clone + Copy + Ord + Default,
{
    pub fn new() -> Self {
        Default::default()
    }
    ...
    pub fn unclaim(&mut self, val: IndexT) {
        if val >= self.next {
            panic!("not a valid entity")
        }

        let is_inserted = self.freed.insert(val);
        assert!(is_inserted, "double-freed")
    }
}
